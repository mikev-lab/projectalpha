...
import { SheetConfig, PDFDocument, PDFPage, ImpositionType, SheetOrientation, PDFFont, PDFImage, ImpositionParams, JobInfoState, SlipSheetColorName } from '../types';
import { 
...
export const usePdfImposition = (params: ImpositionParams): UsePdfImpositionResult => {
  ...
  const drawSlugInfo = async (
    ...
    if (!window.PDFLib || !window.QRious || !inputFile) return;
  ...
  const imposePdf = useCallback(async () => {
    ...
    if (!window.PDFLib || !window.PDFLib.PDFDocument || !window.PDFLib.StandardFonts || !window.QRious) {
      setError("Required libraries (PDFLib, QRious, StandardFonts) not loaded. Please refresh.");
      return;
    }
